...
def _windowed_subdivs(padded_img, window_size, subdivisions, nb_classes, pred_func):
    ...
    subdivs_res = np.zeros((a, b, c, d, nb_classes), dtype=np.float32)
    for i in range(a):
        for j in range(b):
            pred1 = pred_func(subdivs[i][j])[0,:,:,:]
            pred0 = 1-pred1
            subdivs_res[i][j] = np.concatenate((pred0, pred1), axis=2)

    subdivs_res = subdivs_res.reshape(a * b, c, d, nb_classes)
    gc.collect()
    # subdivs = pred_func(subdivs)
    # gc.collect()
    subdivs_res = np.array([patch * WINDOW_SPLINE_2D for patch in subdivs_res])
    # gc.collect()
    #
    # # Such 5D array:
    subdivs_res = subdivs_res.reshape(a, b, c, d, nb_classes)
